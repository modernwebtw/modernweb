...
(function(window) {
    ...
    var Game = {
        ...
        gameOver: function() {
            ...
            // this.ctx.fillText(message, this.c.width / 2 - this.ctx.measureText(message).width / 2, this.c.height /
            //     2 - 50);
            // this.ctx.fillText(message2, this.c.width / 2 - this.ctx.measureText(message2).width / 2, this.c
            //     .height / 2 - 5);
            // this.ctx.font = "bold 16px Lato, sans-serif";
            // this.ctx.fillText(message3, this.c.width / 2 - this.ctx.measureText(message3).width / 2, this.c
            //     .height / 2 + 30);
            $('#modal_game_over').modal({
                backdrop: 'static',
                keyboard: true,
                show: true
            });
    ...
    Player.prototype.draw = function(transparent) {
        // Game.ctx.fillStyle = this.color;
        // Game.ctx.fillRect(this.x, this.y, this.width, this.height);
        Game.ctx.save();
        if (Game.player.invincible && !!transparent) {
            Game.ctx.globalAlpha = 0.2;
        }
        Game.ctx.drawImage(player_image, this.x, this.y - 64, 52, 84);
        Game.ctx.restore();
    };
    ...
    Enemy.prototype.shoot = function() {
        new EnemyBullet(this.x + this.width / 2, this.y, this.color, Game.random(0, Game.score / 50));
    };

    var EnemyBullet = function(x, y, color, level) {
        this.width = 8 + level;
        this.height = 20;
        this.x = x;
        this.y = y;
